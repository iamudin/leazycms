...
class RateLimit
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($request->segment(1) !== admin_path()) {
            ...
            if (config('modules.multisite_enabled')) {
                ...
                if (is_array($disallowedModules) && count($disallowedModules) > 0) {
                    if (config('modules.current.post_type') && in_array(get_post_type(), collect(get_module())->pluck('name')->toArray()) && in_array(get_post_type(), $disallowedModules)) {
                        abort(404);
                    }
                }
            }
        ...
        if (!in_array($request->segment(1), ['favicon.ico', 'logo.webp', 'stats.webp', 'stats.png', 'stats.jpg', 'captcha-image', admin_path()]) && !$request->ajax() && app()->environment('production') && !Route::is('stream') && !Route::is('captcha')) {
        ...
        if (str($response->headers->get('Content-Type'))->lower() == 'text/html; charset=utf-8') {
            ...
            // Inject client side file validator if there is a file input and it is not a CMS layout
            $isCmsLayout = stripos($content, '/backend/css/main.css') !== false;
            if (!$isCmsLayout && (stripos($content, 'type="file"') !== false || stripos($content, "type='file'") !== false)) {
                try {
