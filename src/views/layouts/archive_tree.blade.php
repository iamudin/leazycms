...
<div id="{{ $widgetId }}" class="leazy-archive-calendar"
    ...
    @if(empty($tree))
        <div class="cal-empty-state">
            <i class="fa fa-calendar-times" style="margin-right: 6px; color: var(--cal-text-muted); font-size: 16px;"></i>
            Belum ada arsip kalender untuk {{ $module->title ?? ucfirst($type) }}.
        </div>
    @else
        <div class="cal-card">

            <!-- Judul Widget Arsip -->
            <div class="cal-header-divider"
                style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; padding-bottom: 8px;">
            ...
            <div class="cal-header-divider"
                style="display: flex; align-items: center; justify-content: space-between; gap: 6px; margin-bottom: 12px; padding-bottom: 10px;">
                <button type="button" id="{{ $widgetId }}_btn_prev" class="cal-btn"
                    onclick="window['{{ $widgetId }}_prevPeriod']()" title="Periode Sebelumnya">
                    <i class="fa fa-chevron-left"></i>
                </button>

                <div style="display: flex; align-items: center; gap: 6px;">
                    <!-- Dropdown Bulan (Hanya bulan yang ada data pada tahun terpilih) -->
                    <select id="{{ $widgetId }}_select_month" class="cal-select"
                        onchange="window['{{ $widgetId }}_onMonthChange']()">
                    ...
                    <select id="{{ $widgetId }}_select_year" class="cal-select"
                        onchange="window['{{ $widgetId }}_onYearChange']()">
                        @foreach($years as $y)
                ...
                <button type="button" id="{{ $widgetId }}_btn_next" class="cal-btn"
                    onclick="window['{{ $widgetId }}_nextPeriod']()" title="Periode Berikutnya">
            ...
            <div
                style="display: grid; grid-template-columns: repeat(7, 1fr); gap: 2px; text-align: center; font-weight: 700; font-size: 11px; margin-bottom: 6px;">
            ...
            <!-- Grid Tanggal -->
            <div id="{{ $widgetId }}_grid"
                style="display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px; text-align: center;">
                <!-- Dibuat dinamis oleh JavaScript -->
            </div>

            <!-- Footer: Quick Links & Summary -->
            <div class="cal-footer-divider"
                style="margin-top: 12px; padding-top: 10px; display: flex; flex-direction: column; gap: 6px; font-size: 11.5px;">
                <div style="display: flex; align-items: center; justify-content: space-between; gap: 4px;">
                    <a id="{{ $widgetId }}_month_link"
                        href="{{ url($type . '/archive/' . $defaultYear . '/' . $defaultMonth) }}" class="cal-month-link">
                        <i class="fa fa-calendar-check"></i> <span id="{{ $widgetId }}_month_label">Arsip Bulan
                            {{ $monthNames[$defaultMonth] ?? $defaultMonth }} {{ $defaultYear }}</span>
                    </a>
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; gap: 4px;">
                    <a id="{{ $widgetId }}_year_link" href="{{ url($type . '/archive/' . $defaultYear) }}"
                        class="cal-year-link">
                        <i class="fa fa-folder-open"></i> <span id="{{ $widgetId }}_year_label">Semua Arsip Tahun
                            {{ $defaultYear }}</span>
                    </a>
