...
@section('content')
    ...
    <div class="col-lg-3" style="max-height: 85vh;overflow:auto">
        <style>
            ...
            #appearanceTabs .nav-link:hover {
                color: #495057;
                border-bottom-color: #dee2e6;
            }
            #appearanceTabs .nav-link.active {
                color: #0d6efd;
                border-bottom-color: #0d6efd;
                background-color: transparent;
            }
            #appearanceTabsContent {
        ...
        <ul class="nav nav-tabs nav-justified" id="appearanceTabs" role="tablist">
            <li class="nav-item">
                <a class="nav-link active" id="tentang-tab" data-toggle="tab" href="#tentang" role="tab" aria-controls="tentang" aria-selected="true">
                    <i class="fa fa-info-circle mr-1"></i> Tentang
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="konfigurasi-tab" data-toggle="tab" href="#konfigurasi" role="tab" aria-controls="konfigurasi" aria-selected="false">
                    <i class="fa fa-cogs mr-1"></i> Pengaturan
                </a>
            </li>
        </ul>
        ...
        <div class="tab-content" id="appearanceTabsContent">
            ...
            <div class="tab-pane fade show active" id="tentang" role="tabpanel" aria-labelledby="tentang-tab">
                ...
                @endif
                    <label class="font-weight-bold mb-1">Ganti Template ? {!! help('Perhatian : Ganti Template akan menimpa template yang sedang digunakan') !!}</label>
                ...
                <div style="text-align:center;font-weight:bold;padding:0.5rem">Atau</div>
                @endif
                ...
                <a href="{{ route('appearance.template_store') }}" class="btn btn-info btn-sm w-100 mb-3">
                    <i class="fa fa-cloud-download-alt"></i> Pilih dari Cloud
                </a>

                <h6>Modul Aktif {!! help('Untuk Melihat contoh preview halaman dari masing-masing module, silahkan klik') !!}</h6>
                <div class="accordion mb-3" id="accordionExample" >
                @php 
                    $module = collect(get_module())->where('public', true)->sortBy('position');
            ...
            <!-- Tab Konfigurasi -->
            <div class="tab-pane fade" id="konfigurasi" role="tabpanel" aria-labelledby="konfigurasi-tab">
                ...
                <form action="{{ URL::current() }}" class="template-setting mb-4" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <small>Konten Halaman Utama {!! help('Hanya tersedia jika template menyediakan pilihan halaman utama') !!}</small>
                        ...
                        @foreach ($template_asset as $field)
                            @php
                                ...
                                $colClass = 'col-lg-12';
                                if (in_array($fieldName, ['Logo Title', 'Logo Description'])) {
                                    $colClass = 'col-9 col-lg-9 pr-1';
                                } elseif (in_array($fieldName, ['Logo Title Color', 'Logo Description Color'])) {
                                    $colClass = 'col-3 col-lg-3 pl-1';
                                }
                            @endphp
                        <div class="{{ $colClass }}">
                            ...
                            <select name="{{ $fieldSlug }}" class="form-control form-control-sm"  @if (isset($fieldExtra)) required @endif onchange="previewPreload()">
                                <option value="">Tanpa Preload</option>
                                @foreach($fieldType as $row)
                                <option value="{{ $row }}" {{ $displayVal == $row ? 'selected' : '' }}>{{ str($row)->headline() }}</option>
                                @endforeach
                            </select>
                            ...
                            @elseif($fieldType === 'color')
                                @php
                                    $colorLabel = in_array($fieldName, ['Logo Title Color', 'Logo Description Color']) ? 'Warna' : str($fieldName)->headline();
                                @endphp
                                <small class="font-weight-bold text-dark">{{ $colorLabel }}</small><br>
                                <input type="color" name="{{ $fieldSlug }}" class="form-control form-control-sm" style="width: 100%; max-width: 80px; height: 31px; padding: 2px;" value="{{ $displayVal ?: '#000000' }}">
                    ...
                    <button type="submit" class="submit_form btn w-100 mt-2 btn-sm btn-outline-primary"> <i class="fa fa-save" ></i> Simpan Pengaturan</button>
                </form>
            </div>
        </div>
    </div>
