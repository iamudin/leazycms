...
@section('content')
  <form class="" action="{{URL::full()}}" method="post" enctype="multipart/form-data">
    ...
    <div class="row">
      <div class="col-lg-12">
        <h3 style="font-weight:normal"> <i class="fa fa-user" aria-hidden="true"></i> Akun <button name="save"
            value="true" class="btn btn-primary btn-sm pull-right"> <i class="fa fa-save" aria-hidden></i>
            Perbaharui</button></h3>
        <br>
        ...
        <div class="row">
          <div class="col-lg-4">
            ...
            <div class="form-group">
              ...
              <input accept="image/jpeg,image/png,image/webp" type="file" class="form-control-file photo compress-image"
                name="photo">
            </div>
          </div>
          <div class="col-lg-8">
            <small for="">Nama</small>
            <input required type="text" class="form-control form-control-sm name" name="name" placeholder="Masukkan Nama"
              value="{{$user->name}}">
            <small for="">Email</small>
            <input required type="email" class="form-control form-control-sm email" name="email"
              placeholder="Masukkan Email" value="{{$user->email}}">

            <small for="">Username</small>
            <div class="input-group">
              <input onkeyup="this.value = this.value.replace(/\s+/g, '').toLowerCase();" required type="text"
                class="form-control form-control-sm username" id="username" name="username"
            ...
            <div class="form-group">
              ...
              <small class="text-danger mb-0">Minimal 8 karakter dan di butuhkan Min 1 Kapital, 1 huruf kecil, 1 angka dan
                symbol
                khusus</small>
            </div>
            ...
            @if($user->isAdmin())
              <div class="form-group">
                <small>Izinkan Login Multi</small>
                <input type="checkbox" name="allow_multi_login" @if(get_option('allow_multi_login') == "1") checked @endif>
              </div>
            @endif
          </div>
        </div>
