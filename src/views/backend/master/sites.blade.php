...
@section('content')
    ...
    <td>
  ${item.theme_version}
  ${item.theme_version && item.theme_version.includes("available") 
      ? `<br><button class="btn-toggle-theme btn btn-sm btn-info ml-2" data-status="1">
           Update
         </button>` 
      : ''}
</td>
    <td>
      <button class="btn-toggle-route btn btn-sm ${item.route_cached ? 'btn-success' : 'btn-danger'}" 
              data-status="${item.route_cached ? 1 : 0}">
        ${item.route_cached ? 'CACHED' : 'UNCHACHED'}
      </button>
    </td>
    <td>
    ...
    <td>
      <button class="btn-login btn btn-sm btn-warning" 
        ...
        Login
                ...
                // bind toggle route cached
                $(".btn-toggle-route").click(function() {
                    let row = $(this).closest("tr");
                    let id = row.data("id");
                    let status = $(this).data("status");
                    toggleAction('cacheroute', id, status, this);
                });

                // bind toggle theme cached
                $(".btn-toggle-theme").click(function() {
                    let row = $(this).closest("tr");
                    let id = row.data("id");
                    let status = $(this).data("status");
                    toggleAction('updatetheme', id, status, this);
                });
        ...
        $(document).on('click', '.btn-login', function(e) {
            ...
            $.getJSON('{{ route('app.master.fetch') }}', {
                    type: 'autoauth',
                    id: id,
                    token: api_key
                })
