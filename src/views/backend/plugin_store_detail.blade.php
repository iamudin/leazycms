@extends('cms::backend.layout.app', ['title' => 'Detail Plugin: ' . $template['name']])
...
<style>
    ...
    .desc-title::after {
        ...
        width: 40px;
        height: 3px;
        background: var(--theme-primary, #20c997);
        border-radius: 2px;
    }
    .desc-content {
        color: #475569;
...
<div class="row mb-4">
    <div class="col-12">
        <a href="{{ route('admin.plugins.store') }}" class="btn btn-light shadow-sm" style="border-radius: 30px; padding: 8px 20px; font-weight: 600;">
            <i class="fa fa-arrow-left mr-2"></i> Kembali ke Plugin Store
        </a>
    </div>
</div>
    ...
    <div class="col-lg-8 mb-4">
        <div class="detail-card">
            ...
            <div class="detail-body">
                ...
                <h4 class="desc-title">Deskripsi Plugin</h4>
                <div class="desc-content mt-3">
                    {{ $template['description'] ?? 'Tidak ada deskripsi rinci untuk plugin ini.' }}
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    $('.install-cloud-btn').click(function() {
        ...
        if (confirm('Lanjutkan install plugin ini?')) {
    ...
    $('.toggle-plugin-btn').click(function() {
        ...
        if (confirm('Yakin ingin ' + (action == 'enable' ? 'mengaktifkan' : 'menonaktifkan') + ' plugin ini?')) {
            $(this).prop('disabled', true).html('<i class="fa fa-spinner fa-spin mr-2"></i> Memproses...');
