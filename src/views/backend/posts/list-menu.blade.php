...
<div class="row">
  <div class="col-lg-12">
        ...
        @foreach(json_decode(json_encode($menu->where('menu_parent', 0))) as $y => $l)
          ...
          <li class="dd-item dd3-item menu-id-{{$l->menu_id}}" data-id="{{$l->menu_id}}">
            ...
            <div class="dd3-content">{{$l->menu_name}} <i class="fa fa-angle-right" aria-hidden></i>
              <code><a href="{{link_menu($l->menu_link)}}" title="Klik untuk mengunjungi"><i>{{Str::limit(link_menu($l->menu_link), '60', '...')}}</i></a></code><span
                style="float:right"><a href="javascript:void(0)"
                  onclick="$('.description').val('{{$l->menu_description}}');$('.link').val('{{$l->menu_link}}');$('.name').val('{{$l->menu_name}}');$('.iconx').val('{{$l->menu_icon}}');$('#type').val('{{$l->menu_id}}');$('.modal').modal('show')"
    ...
    <button type="button" class="btn btn-sm btn-info pull-right btnadd"
      onclick="$('#type').val('add');$('.menu').val('');$('textarea').val('');$('.modal').modal('show');" name="button">
    ...
    <div class="modal-content">
      ...
      <form class="" action="javascript:void(0)" method="post">
        ...
        <div class="modal-footer">
          <button class="btn btn-primary save" onclick="setmenu();$('.modal').modal('hide')" type="button" name="save"
            value="">Simpan</button>
