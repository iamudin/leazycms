...
@if(!empty($l?->$k) && media_exists($l?->$k))
    <div class="media-preview-wrapper d-inline-flex align-items-center">
        ...
        @if(!Route::is($post->type . '.show'))
            <i class="fa fa-trash pointer text-danger edit-{{ _us($r[0]) }}-{{ $y }} btn-remove-media" style="cursor: pointer; display: none; font-size: 12px;" onclick="$(this).closest('.media-preview-wrapper').hide().prev('.media-input-wrapper').show().find('input[type=hidden]').val('nofile')"></i>
        @endif
    </div>
@endif
