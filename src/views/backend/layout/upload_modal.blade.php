...
<script>
    ...
    function selectMediaGridItem(elem, event) {
        ...
        let isMultiple = currentFileInput && currentFileInput.prop('multiple');

        if (isMultiple) {
            $(elem).toggleClass('border-primary shadow selected-media');
            if ($(elem).hasClass('selected-media')) {
            ...
            $('.media-grid-item').removeClass('border-primary shadow selected-media').css('border-width', '1px');
            $(elem).addClass('border-primary shadow selected-media').css('border-width', '2px');
        ...
        $('#btn-use-selected-media').on('click', function () {
            ...
            if (currentFileInput && currentFileWrapper) {
                ...
                vals.forEach(function(val) {
                    ...
                    let itemDiv = $('<div style="display:inline-block; margin-right:5px; margin-bottom:5px; text-align:center; vertical-align:top;"></div>');

                    if (isImage) {
                        let url = '/media/' + val + (!['svg', 'ico'].includes(ext) ? '?size=small' : '');
                        let $img = $('<img>').attr('src', url).css({
                            'width': vals.length > 1 ? '60px' : '100%',
                            'height': vals.length > 1 ? '60px' : 'auto',
                            'max-height': '150px',
                            'object-fit': 'contain',
                            'margin-bottom': '5px',
                            'border-radius': '4px'
